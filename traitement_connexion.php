...
//     $query = "UPDATE utilisateur SET mdp_user = :password WHERE email_user = :mail";
//     $stmt = $pdo->prepare($query);
...
//     $query = "SELECT * FROM utilisateur WHERE email_user = :mail LIMIT 1";
//     $stmt = $pdo->prepare($query);
//     $stmt->bindParam(':mail', $email);
//     $stmt->execute();
...
if($email&&$password){
    ...
    try {
        ...
        if($data_user && password_verify($password, $data_user["password"]) ){
            ...
            window.location.href = "accueil_2.php";
          </script>';
    exit();
